...
tss.16.gr <- makeGRangesFromDataFrame(tss.16.s.trunc,keep.extra.columns = TRUE,seqnames.field="seq",start.field="TSS",end.field="TSS",strand.field = "strand")

tss.17.gr <- makeGRangesFromDataFrame(tss.17.s.trunc,keep.extra.columns = TRUE,seqnames.field="seq",start.field="TSS",end.field="TSS",strand.field = "strand")

tss.18.gr <- makeGRangesFromDataFrame(tss.18.s.trunc,keep.extra.columns = TRUE,seqnames.field="seq",start.field="TSS",end.field="TSS",strand.field = "strand")
...
tsr.16.gr <- makeGRangesFromDataFrame(tsr.16.s.trunc,keep.extra.columns = TRUE, seqnames.field="seq",start.field="start",end.field="end",strand.field = "strand")

tsr.17.gr <- makeGRangesFromDataFrame(tsr.17.s.trunc,keep.extra.columns = TRUE, seqnames.field="seq",start.field="start",end.field="end",strand.field = "strand")

tsr.18.gr <- makeGRangesFromDataFrame(tsr.18.s.trunc,keep.extra.columns = TRUE, seqnames.field="seq",start.field="start",end.field="end",strand.field = "strand")
...
#Truncated Genome Troubleshooting 

#do for each tss and tsr then run makeGRanges, then make the exp

tss.17.s <- tss.17[mixedorder(tss.17$seq),]

test.ind <- which(tss.17.s$seq == "scaffold_388") #scaffold # varies.. >tail(tss.17.s.trunc, n=30) to determine where to cut off

tss.17.s.trunc <- tss.17.s[1:(test.ind[1]-1),]

tss.17.s.trunc <- tss.17.s[1:(test.ind[1]-1),]


#making the exp files with all three replicates
TEX36.tss <- list(tss.16.gr, tss.17.gr, tss.18.gr) 
names(TEX36.tss) <- c("TEX36_r1", "TEX36_r2", "TEX36_r3")

TSRs_TEX36 <- list(tsr.19.gr, tsr.20.gr, tsr.21.gr)
names(TSRs_TEX36) <- c("TEX36_r1", "TEX36_r2", "TEX36_r3")


exp <- tsr_explorer(TEX36.tss, TSRs_TEX36)
...
TEX36.tss <- list(tss.16.gr, tss.18.gr)
names(TEX36.tss) <- c("TEX36_r1", "TEX36_r2")

TSRs_TEX36 <- list(tsr.16.gr, tsr.18.gr)
names(TSRs_TEX36) <- c("TEX36_r1", "TEX36_r2")

exp <- tsr_explorer(TEX36.tss, TSRs_TEX36)
exp <- count_normalization(exp2, data_type = "tss", threshold = 3, n_samples = 1)

p <- plot_correlation(exp, data_type = "tss") +
  ggplot2::theme_bw() +
  ggplot2::theme(text = element_text(size = 6))

ggsave("tss_correlation_TEX36.png", plot = p, device = "png", type = "cairo", height = 3, width = 3)

#TSS Motif and Base Composition 
seqs <- tss_sequences(exp, genome_assembly = Cz.assembly, threshold = 3)

p <- plot_sequence_logo(seqs, ncol = 3) + png("tss_seq_logo_quantiles_TEX36.png", units = "in", res = 300, height = 1, width = 6, type = "cairo")
p
dev.off()
...
ggsave("tss_dinucleotide_frequencies_TEX36.png", plot = p, device = "png", type = "cairo", height = 2, width = 5)
